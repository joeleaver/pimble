//! **Legacy** Automerge-backed store document.
//!
//! This exists **only** to read `store.automerge` files written before the Phase B
//! migration to a yrs-backed [`crate::store_document::StoreDocument`]. It is a
//! byte-for-byte move of the pre-migration implementation (renamed
//! `StoreDocument` -> `LegacyStoreDocument`) so that migration code and tests can
//! `load` an old document and hand it to
//! [`crate::store_document::StoreDocument::from_legacy`]. Do not add new features here;
//! new store-document work happens in `store_document.rs`.
...
use crate::store_document::{NodeInfo, TreeIssue};
...
#[doc(hidden)]
#[derive(Debug)]
pub struct LegacyStoreDocument {
    doc: AutoCommit,
}

impl LegacyStoreDocument {
    ...
    pub fn merge(&mut self, other: &mut LegacyStoreDocument) -> Result<()> {
...
mod tests {
    ...
    fn test_new_store_document() {
        ...
        let doc = LegacyStoreDocument::new("Test Store", root_id).unwrap();
    ...
    fn test_add_and_get_node() {
        let root_id = NodeId::new();
        let mut doc = LegacyStoreDocument::new("Store", root_id).unwrap();
    ...
    fn test_save_load() {
        ...
        let bytes = doc.save();
        let loaded = LegacyStoreDocument::load(&bytes).unwrap();
    ...
    fn test_concurrent_merge() {
        let root_id = NodeId::new();
        let mut doc1 = LegacyStoreDocument::new("Store", root_id).unwrap();
