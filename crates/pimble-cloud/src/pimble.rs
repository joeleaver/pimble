...
impl PimbleService {
    ...
    /// Create a new hosted store named `name` at a fresh path inside the
    /// configured stores directory. Returns the Pimble server's own
    /// `StoreId` and the directory name it was created under (what
    /// `HostedStore::dir_name` records).
    pub async fn create_store(&self, name: &str) -> CloudResult<(StoreId, String)> {
        let dir_name = format!("{}.pimble", uuid::Uuid::new_v4());
        let path = self.stores_dir.join(&dir_name);
        let (store_id, _root_node_id) = self.client.create_store(&path, name).await?;
