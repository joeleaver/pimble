...
- `rinch` (git main), declared in `crates/pimble-app/Cargo.toml`: default features off, `native` adds `desktop, gpu, file-dialogs, clipboard, debug, collaboration`, `web` adds `rinch-web` with `collaboration`. The desktop renders on the GPU (vello over wgpu): the CPU renderer repaints the whole window after every layout change, about 100 ms a frame at 4K. `gpu` needs rinch's wgpu and winit forks, so the root `Cargo.toml` repeats rinch's `[patch.crates-io]` block; keep the two identical. The software renderer is in the same binary: `pimble --cpu` (or `--renderer cpu`, or `RINCH_RENDERER=cpu`) presents with it, and when the GPU will not start the window falls back to it with a warning in the log (rinch `App::renderer`, `Renderer::Auto`; `--renderer gpu` panics instead). Until joeleaver/rinch#857, #858, #869 and #871 are merged, every rinch dependency (root, `crates/pimble-app`, `web/`) points at rinch's `pimble/perf` branch, which merges them; move all of them back to `main` together
